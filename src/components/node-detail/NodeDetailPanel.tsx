...
import { Info, Terminal, Activity, X, Cpu } from 'lucide-react';
...
const NodeDetailPanel: React.FC<NodeDetailPanelProps> = ({ nodeId }) => {
    const [activeTab, setActiveTab] = useState<'info' | 'config' | 'interfaces'>('info');
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="bg-node/50 p-6 border-b border-node-border">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-black tracking-tight text-white uppercase italic">{device.hostname}</h3>
                ...
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`flex flex-1 items-center justify-center gap-2 py-4 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab.id
                                ...
                                : 'text-gray-500 hover:bg-node/50 hover:text-white'
                            }`}
                    >
                        <tab.icon size={14} />
                        {tab.label}
